...
private:
	...
	int head_direction, head_speed, snake_parts_add,pause_state,current_state;
...
public:
	...
	void move_fruit() {
		bool bad_pos = false;
		sf::Vector2f apple_res = sf::Vector2f(window_res.at(0) / 20 - 2, window_res.at(1) / 20 - 2);
		sf::Vector2f updated_apple_pos;
		srand(time(nullptr));
		do {
			 bad_pos = false;
			 updated_apple_pos.x = (float)(1+rand() / ((RAND_MAX + 1u) / (int)apple_res.x)) * 20;
			 updated_apple_pos.y = (float)(1+rand() / ((RAND_MAX + 1u) / (int)apple_res.y)) * 20;

			 for (int i = 0 ; i < snake.size(); i++)
			 	if (snake.at(i).get_shape().getGlobalBounds().intersects
			 		(sf::Rect<float>(updated_apple_pos.x,updated_apple_pos.y,20,20))) {
			 		bad_pos = true;
			 		break;
			 }
	...
	void run(){
		...
		while(window.isOpen()){
			...
			if (current_state == State::PAUSED || current_state == State::ENDGAME) {
